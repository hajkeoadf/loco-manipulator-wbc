...
class Runner:
    ...
    def __init__(self, env, device='cpu', run_name: str=None, resume=False, log_dir=None, debug=False):
        ...
        if self.log_dir is not None:
            self.tb_writer = SummaryWriter(log_dir=osp.join(self.log_dir, 'tensorboard'))
        else:
            self.tb_writer = None
        ...
        for it in range(self.current_learning_iteration, tot_iter):
            ...
            if self.log_dir is not None:
                ...
                if self.tb_writer is not None:
                    self.tb_writer.add_scalar("Efficiency/collect_time", collection_time, it)
                    self.tb_writer.add_scalar("Efficiency/learn_time", learn_time, it)
                ...
                for key in ep_infos[0].keys():
                    ...
                    if not self.debug and self.tb_writer is not None:
                        self.tb_writer.add_scalar('Train_Reward_episode/' + key, mean, it)
                ...
                # ✅ 使用tensorboard记录训练损失
                if not self.debug and self.tb_writer is not None:
                    self.tb_writer.add_scalar("Train_Loss/mean_value_loss_arm", mean_value_loss_arm, it)
                    self.tb_writer.add_scalar("Train_Loss/mean_surrogate_loss_arm", mean_surrogate_loss_arm, it)
                    self.tb_writer.add_scalar("Train_Loss/mean_adaptation_module_loss_arm", mean_adaptation_module_loss_arm, it)
                    
                    self.tb_writer.add_scalar("Train_Loss/mean_value_loss_dog", mean_value_loss_dog, it)
                    self.tb_writer.add_scalar("Train_Loss/mean_surrogate_loss_dog", mean_surrogate_loss_dog, it)
                    self.tb_writer.add_scalar("Train_Loss/mean_adaptation_module_loss_dog", mean_adaptation_module_loss_dog, it)

                    self.tb_writer.add_scalar("Train_std/arm_action_std", arm_action_std.mean(), it)
                    self.tb_writer.add_scalar("Train_std/dog_action_std", dog_action_std.mean(), it)
                    
                    # 记录总奖励
                    if len(rewbuffer) > 0:
                        self.tb_writer.add_scalar('Train_Total_Reward/mean_reward', statistics.mean(rewbuffer), it)
                        self.tb_writer.add_scalar('Train_Total_Reward/mean_episode_length', statistics.mean(lenbuffer), it)
                    
                    # ✅ 记录学习率
                    if hasattr(self.alg_arm, 'learning_rate'):
                        self.tb_writer.add_scalar("Train/Learning_Rate_arm", self.alg_arm.learning_rate, it)
                    if hasattr(self.alg_dog, 'learning_rate'):
                        self.tb_writer.add_scalar("Train/Learning_Rate_dog", self.alg_dog.learning_rate, it)
                    
                    # ✅ 记录FPS
                    self.tb_writer.add_scalar("Efficiency/FPS", fps, it)
                    self.tb_writer.add_scalar("Efficiency/iteration_time", iteration_time, it)
    ...
    def save_dog(self, it):
        ...
        # ✅ 记录模型保存到tensorboard
        if self.tb_writer is not None:
            self.tb_writer.add_text("Model/Checkpoint_dog", f"Saved checkpoint at iteration {it}", it)
    ...
    def save_arm(self, it):
        ...
        # ✅ 记录模型保存到tensorboard
        if self.tb_writer is not None:
            self.tb_writer.add_text("Model/Checkpoint_arm", f"Saved checkpoint at iteration {it}", it)
